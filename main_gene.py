        ...
        for i, j in route_aff:
            ...
            for k in tinds:
                ...
                if self.dis[pre_node][ver] < self.edge_max and self.dis[post_node][ver] < self.edge_max \
                    and tdis - self.dis[pre_node][post_node] + self.dis[pre_node][ver] + self.dis[post_node][ver] < self.loop_max:
                    #满足要求，在k和k+1之间加入ver点
    ...
    def ans_analysis(self, unit):
        ...
        for i in (unit.keys() - [0]):
            ...
            for j in (unit[i].keys() - ['num', 'arranged']):
                print("   路线：", unit[i][j][0], "  运载量：{:.2f}  路线长度：{:.2f} km  耗时：{:.2f} min".format(unit[i][j][1], unit[i][j][2], unit[i][j][3]))
        ...
        unit['routes'] = []
        for i in (unit.keys() - ['overall', 'routes']):
            tem = unit.pop(i)
            tem['load_cap'] = i
            unit['routes'].append(tem)
...
if __name__ == '__main__':
    ...
    v.solve(start_num = 150, max_iter = 40, max_population = 1000, fac_full = 5, 
            fac_distance = 4, fac_time = 3, show = True, dia = True)
