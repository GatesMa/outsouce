...
class vrp():
    def __init__(self,csv_name,data_type):
        #数据读入，参数是文件名和读入类型，
        
        with open(csv_name,encoding = 'utf-8-sig') as fi:
            t = csv.reader(fi)
            mdata = [i for i in t]#源数据
        self.num = int(mdata[0][0]) #num是送货点的个数，num+1是所有点个数
        
        if data_type == 'dis':    #类型为dis，说明位置是以距离矩阵的形式给出
            self.dis = list(map(lambda x:x[0:self.num + 1],mdata[1:self.num + 2]))#list不支持二维切片，只能这样切出距离矩阵
            for i in self.dis:
                for j in range(len(i)):
                    i[j] = float(i[j])#字符串转成浮点
                    
        elif data_type == 'coo':  #类型为coo，位置以坐标形式给
            coo = list(map(lambda x:x[0:2],mdata[1:self.num + 2]))
            for i in coo:
                for j in range(len(i)):
                    i[j] = float(i[j])
            self.dis = tran(coo)#坐标矩阵到距离矩阵
        else:
            raise Exception('csv input type error')
            
        self.dem = [float(i) for i in mdata[self.num + 2][0: self.num + 1]]#读入需求，注意0位存0，这样dem[i]表示i号需求
        self.carLoad = {}
        for i in range(int(mdata[self.num + 3][0])):
            self.carLoad[float(mdata[self.num + 3][2 * i + 1])] =\
            int(mdata[self.num + 3][2 * i + 2])
            #这里注意，一定是 载量：数量
        self.edge_max = float(mdata[self.num + 4][0]) #单边最大
        self.loop_max = float(mdata[self.num + 4][1]) #回路最大
        self.speed = float(mdata[self.num + 4][2])/60.0 #车速 km/min
        self.uTime = float(mdata[self.num + 4][3]) #卸货时间 /min
        self.tem = [self.num,self.dis,self.dem,self.carLoad,self.edge_max,self.loop_max,self.speed,self.uTime] 
        #临时变量，方便查看读取情况
        # print(type(self.dis[1][1]))
        #
        # print(type(self.num))
        # print(type(self.dis))
        # print(type(self.dem))
        # print(type(self.carLoad))
        # print(type(self.edge_max))
        # print(type(self.loop_max))
        # print(type(self.speed))
        # print(type(self.uTime))
    ...
    def get_ans(self):
        ...
        # for i in tem_keys:
        #     car2route[(i, cars[i])] = car2route[i]
        #     car2route.pop(i)
        #
...
if __name__ == '__main__':
    ...
    # print(type(v.dis))
    # print(type(v.dem))
    # print(type(v.carLoad))
    # print(type(v.edge_max))
    # print(type(v.loop_max))
    # print(type(v.speed))
    # print(type(v.uTime))
